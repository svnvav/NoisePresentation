...
static uint GradientsMask3D = 31;
static float SimplexScale3D = 37.83723;
...
float SimplexGradientPart(float3 p, int3 i)
{
	float coeffUnskew = Factors.w * dot(i, Skewers.xxx);
	float3 unskewed = p - i + coeffUnskew;
	float3 g = Gradients[Hashes[Hashes[Hashes[i.x & HashMask] + i.y & HashMask] + i.z & HashMask] & GradientsMask3D];
	float v = dot(g, unskewed);
	return FallOff(unskewed) * v;
}
...
float Noise(float3 position, uint scale)
{
	...
	float sample = SimplexGradientPart(pos, i);
	sample += SimplexGradientPart(pos, i + 1);
	...
	if (x >= y) {
		if (x >= z) {
			sample += SimplexGradientPart(pos, int3(i.x + 1, i.y, i.z));
			if (y >= z) {
				sample += SimplexGradientPart(pos, int3(i.x + 1, i.y + 1, i.z));
			}
			else {
				sample += SimplexGradientPart(pos, int3(i.x + 1, i.y, i.z + 1));
			}
		}
		else {
			sample += SimplexGradientPart(pos, int3(i.x, i.y, i.z + 1));
			sample += SimplexGradientPart(pos, int3(i.x + 1, i.y, i.z + 1));
		}
	}
	else {
		if (y >= z) {
			sample += SimplexGradientPart(pos, int3(i.x, i.y + 1, i.z));
			if (x >= z) {
				sample += SimplexGradientPart(pos, int3(i.x + 1, i.y + 1, i.z));
			}
			else {
				sample += SimplexGradientPart(pos, int3(i.x, i.y + 1, i.z + 1));
			}
		}
		else {
			sample += SimplexGradientPart(pos, int3(i.x, i.y, i.z + 1));
	...
	//float key0 = Gradients[Hashes[Hashes[Hashes[i.x & HashMask] + i.y & HashMask] + i.z & HashMask] & GradientsMask3D];
	
	return sample * SimplexScale3D;//lerp(key0, sample * SimplexScale3D, Factors.z);
}
